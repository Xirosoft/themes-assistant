    ...
    class AtaInstaller{
        ...
        */
        public function ata_config_table(){
            global $wpdb;
            $ata_submissions_table = $wpdb->prefix . 'ata_settings';
        
            // Check if the table exists before creating it
            if (!$this->ata_is_table_exists($ata_submissions_table)) {
                $charset_collate = $wpdb->get_charset_collate();
                $sql = "CREATE TABLE $ata_submissions_table (
                ...
                $existing_data = $wpdb->get_row( $wpdb->prepare($query, $ata_submissions_table));
                // Insert data into the table after creating it
                $ata_elements_settings_data = '{"form_option_checkbox":"on","form_option_date":"on","form_option_files":"on","form_option_header":"on","form_option_hidden":"on","form_option_number":"on","form_option_paragraph":"on","form_option_radio":"on","form_option_select":"on","form_option_text":"on","form_option_textarea":"on","form_attr_description":"on","form_attr_name":"on","form_attr_maxlength":"on","form_attr_style":"on","form_attr_class":"on","form_attr_label":"on","form_attr_placeholder":"on","form_attr_value":"on","form_attr_type":"on","form_attr_required":"on"}';

                if (!$existing_data) {
                    // Insert data if no existing data
                    $data = array(
                        'elements_settings' => wp_json_encode($ata_elements_settings_data),
                        'addon_setting' => 'coming soon',
                    );
        
                    $wpdb->insert(
                        $ata_submissions_table,
                        $data
                    );
                } else {
        ...
        /**
        * ata_is_table_exists function just checking exists table.
        *
        * @param [string] $table_name
        * @return boolean
        */
        function ata_is_table_exists($table_name) {
            global $wpdb;
